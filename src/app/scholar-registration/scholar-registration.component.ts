...
export class ScholarRegistrationComponent implements OnInit {
    ...
    this.apiservice.getActiveSemester().subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        ...
        this.errorMessage.set(
          err.error?.message ||
            'Server error occurred while fetching active semester'
        );
      },
    });
  }
  ...
  openRegistrationTab() {
    ...
    if (!this.isRegistrationDateAvailable) {
      this.errorMessage.set('There is no semester available for registration');
      return;
    }
  ...
  registerSemester() {
    ...
    this.apiservice.RegisterSemester(this.registrationForm).subscribe({
      next: (res) => {
